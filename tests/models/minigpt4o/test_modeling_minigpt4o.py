...
"""Testing suite for the PyTorch Minigpt4o model."""
...
from ..minigpt4o.test_modeling_minigpt4o import Minigpt4oModelTester
...
class Minigpt4oAudioModelTester:
    def __init__(
        self,
        parent,
        batch_size=2,
    ...
    def prepare_config_and_inputs_for_common(self):
        ...
        feature_extractor = Minigpt4oAudioFeatureExtractor(**feature_extractor_config)
...
@require_torch
class Minigpt4oAudioModelTest(ModelTesterMixin, unittest.TestCase):
    all_model_classes = (Minigpt4oAudioEncoder,) if is_torch_available() else ()
    test_pruning = False
    ...
    def setUp(self):
        self.model_tester = Minigpt4oAudioModelTester(self)
        self.config_tester = ConfigTester(self, config_class=Minigpt4oAudioConfig, hidden_size=37)
        torch.manual_seed(0)
        ...
        self.feature_extractor = Minigpt4oAudioFeatureExtractor(**self.model_tester.get_feature_extractor_config())
        np.random.seed(0)
    ...
    def test_audio_encoder(self):
        ...
        model = Minigpt4oAudioEncoder(config).to(torch_device).eval()
...
class Minigpt4oTextModelTester(Minigpt4oModelTester):
    ...
    if is_torch_available():
        config_class = Minigpt4oTextConfig
        model_class = Minigpt4oTextModel
        for_causal_lm_class = Minigpt4oForCausalLM


@require_torch
class Minigpt4oTextModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    all_model_classes = (Minigpt4oTextModel, Minigpt4oForCausalLM) if is_torch_available() else ()
    all_generative_model_classes = (Minigpt4oForCausalLM,) if is_torch_available() else ()
    ...
    def setUp(self):
        self.model_tester = Minigpt4oTextModelTester(self)
        self.config_tester = ConfigTester(
            self,
            config_class=Minigpt4oConfig,
            hidden_size=37,
    ...
    def _check_hidden_states_for_generate(
        ...
        "Minigpt4o has special hidden states shape with 1 additional dim (which is then reduced with projections)"
    ...
    @unittest.skip(
        "Minigpt4o has a special shape for hidden states (due to per-layer projs) which is not compatible with contrastive decoding"
    )
    ...
    @pytest.mark.generate
    @unittest.skip(
        "Minigpt4o has a special shape for hidden states (due to per-layer projs) which is not compatible with dola decoding"
    )
    def test_dola_decoding_sample(self):
        pass

    @pytest.mark.generate
    @unittest.skip("Minigpt4o does not support QuantizedCache as it performs cache manipulation in the forward pass")
    def test_generate_with_quant_cache(self):
        pass


class Minigpt4oVision2TextModelTester:
    text_config = {"activation_sparsity_pattern": None}
    ...
    def __init__(
        ...
        self.llm_tester = Minigpt4oTextModelTester(self.parent)
...
@unittest.skip("Skipped for now!")
@require_torch
class Minigpt4oVision2TextModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    all_model_classes = (Minigpt4oModel, Minigpt4oForConditionalGeneration) if is_torch_available() else ()
    all_generative_model_classes = (Minigpt4oForConditionalGeneration,) if is_torch_available() else ()
    test_headmasking = False
    test_pruning = False
    test_missing_keys = False
    _is_stateful = True
    ...
    def setUp(self):
        self.model_tester = Minigpt4oVision2TextModelTester(self)
        self.config_tester = ConfigTester(
            self,
            config_class=Minigpt4oConfig,
            hidden_size=37,
    ...
    @pytest.mark.generate
    @unittest.skip("Minigpt4o has HybridCache which is not compatible with assisted decoding")
    ...
    @unittest.skip("Minigpt4o has HybridCache which is not compatible with dola decoding")
    def test_dola_decoding_sample(self):
        pass

    @unittest.skip("Minigpt4o has HybridCache and doesn't support continue from past kv")
    def test_generate_continue_from_past_key_values(self):
        pass

    @unittest.skip("Minigpt4o has HybridCache and doesn't support low_memory generation")
    def test_beam_search_low_memory(self):
        pass

    @unittest.skip("Minigpt4o has HybridCache and doesn't support contrastive generation")
    def test_contrastive_generate(self):
        pass
    ...
    @unittest.skip("Minigpt4o has HybridCache and doesn't support StaticCache. Though it could, it shouldn't support.")
    ...
    def test_automodelforcausallm(self):
        """
        Regression test for #36741 -- make sure `AutoModelForCausalLM` works with a Minigpt4o config, i.e. that
        `AutoModelForCausalLM.from_pretrained` pulls the text config before loading the model
        """
        config = self.model_tester.get_config()
        model = Minigpt4oForConditionalGeneration(config)
        with tempfile.TemporaryDirectory() as tmp_dir:
            model.save_pretrained(tmp_dir)
            for_causal_lm = AutoModelForCausalLM.from_pretrained(tmp_dir)
            self.assertIsInstance(for_causal_lm, Minigpt4oForCausalLM)
...
@require_torch_gpu
@require_read_token
class Minigpt4oIntegrationTest(unittest.TestCase):
    def setUp(self):
        self.processor = AutoProcessor.from_pretrained("Google/minigpt4o-E4B-it", padding_side="left")
    ...
    def test_model_4b_bf16(self):
        model_id = "Google/minigpt4o-E4B-it"
    ...
    def test_model_1b_text_only(self):
        model_id = "google/minigpt4o-3-1b-it"

        model = Minigpt4oForCausalLM.from_pretrained(model_id, low_cpu_mem_usage=True, torch_dtype=torch.bfloat16).to(
            torch_device
        )
