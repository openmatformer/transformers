...
from .configuration_minigpt4o import Minigpt4oAudioConfig, Minigpt4oConfig, Minigpt4oTextConfig, Minigpt4oVisionConfig
...
@auto_docstring(
    custom_intro="""
    Base class for Minigpt4o outputs, with hidden states and attentions.
    """
)
...
@auto_docstring(
    custom_intro="""
    Base class for Minigpt4o causal language model (or autoregressive) outputs.
    """
)
class Minigpt4oCausalLMOutputWithPast(ModelOutput):
    r"""
...
class Minigpt4oAudioRelativePositionEmbedding(nn.Module):
    def __init__(self, config: Minigpt4oAudioConfig):
        super().__init__()
        self.config = config
...
class Minigpt4oAudioAttention(nn.Module):
    def __init__(self, config: Minigpt4oAudioConfig):
        super().__init__()
        self.config = config
        ...
        self.relative_position_embedding = Minigpt4oAudioRelativePositionEmbedding(config)
...
class Minigpt4oAudioCumulativeGroupNorm(nn.Module):
...
class Minigpt4oAudioSSCPConvBlock(nn.Module):
    ...
    def __init__(
        ...
        self.norm = Minigpt4oAudioCumulativeGroupNorm(
...
class Minigpt4oAudioSubSampleConvProjection(nn.Module):
    def __init__(self, config: Minigpt4oAudioConfig):
        super().__init__()
        self.config = config
...
class Minigpt4oAudioConformerAttention(nn.Module):
    def __init__(self, config: Minigpt4oAudioConfig):
        super().__init__()
        self.config = config
        ...
        self.pre_attn_norm = Minigpt4oRMSNorm(self.config.hidden_size)
        self.attn = Minigpt4oAudioAttention(config)
        self.post = nn.Linear(self.post_in_features, self.config.hidden_size, bias=False)
        self.post_norm = Minigpt4oRMSNorm(self.config.hidden_size)
...
class Minigpt4oAudioConformerFeedForward(nn.Module):
    def __init__(self, config: Minigpt4oAudioConfig):
        super().__init__()
        self.config = config
        ...
        self.pre_layer_norm = Minigpt4oRMSNorm(self.config.hidden_size)
        ...
        self.post_layer_norm = Minigpt4oRMSNorm(self.config.hidden_size)
...
class Minigpt4oAudioConformerLightConv1d(nn.Module):
    def __init__(self, config: Minigpt4oAudioConfig):
        super().__init__()
        self.config = config

        self.pre_layer_norm = Minigpt4oRMSNorm(self.config.hidden_size, eps=self.config.rms_norm_eps)
        ...
        self.register_buffer("gradient_clipping", torch.tensor(self.config.gradient_clipping), persistent=False)
        self.conv_norm = Minigpt4oRMSNorm(self.config.hidden_size, eps=self.config.rms_norm_eps)
...
class Minigpt4oAudioConformerBlock(nn.Module):
    def __init__(self, config: Minigpt4oAudioConfig):
        super().__init__()
        self.config = config

        self.ffw_layer_start = Minigpt4oAudioConformerFeedForward(self.config)
        self.attention = Minigpt4oAudioConformerAttention(self.config)
        self.lconv1d = Minigpt4oAudioConformerLightConv1d(self.config)
        self.ffw_layer_end = Minigpt4oAudioConformerFeedForward(self.config)
        self.register_buffer("gradient_clipping", torch.tensor(self.config.gradient_clipping), persistent=False)
        self.norm = Minigpt4oRMSNorm(self.config.hidden_size)
...
class Minigpt4oAudioEncoder(PreTrainedModel):
    ...
    def __init__(self, config: Minigpt4oAudioConfig):
        ...
        self.subsample_conv_projection = Minigpt4oAudioSubSampleConvProjection(config)
        self.conformer = nn.ModuleList(
            [Minigpt4oAudioConformerBlock(config) for _ in range(config.conf_num_hidden_layers)]
        )
...
class Minigpt4oTextScaledWordEmbedding(nn.Embedding):
    """
...
class Minigpt4oTextLaurelBlock(nn.Module):
...
class Minigpt4oTextMLP(nn.Module):
...
class Minigpt4oTextAltUp(nn.Module):
...
class Minigpt4oTextRotaryEmbedding(nn.Module):
    def __init__(self, config: Minigpt4oTextConfig, device=None):
        super().__init__()
...
class Minigpt4oTextAttention(nn.Module):
    ...
    def __init__(self, config: Minigpt4oTextConfig, layer_idx: int):
        ...
        self.q_norm = Minigpt4oRMSNorm(dim=config.head_dim, eps=config.rms_norm_eps)
        self.k_norm = Minigpt4oRMSNorm(dim=config.head_dim, eps=config.rms_norm_eps)
        self.v_norm = Minigpt4oRMSNorm(dim=config.head_dim, eps=config.rms_norm_eps, with_scale=False)
...
class Minigpt4oTextDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: Minigpt4oTextConfig, layer_idx: int):
        super().__init__()
        self.config = config
        self.hidden_size = config.hidden_size
        ...
        self.self_attn = Minigpt4oTextAttention(config, layer_idx)
        self.mlp = Minigpt4oTextMLP(config, layer_idx=layer_idx)
        self.input_layernorm = Minigpt4oRMSNorm(self.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = Minigpt4oRMSNorm(self.hidden_size, eps=config.rms_norm_eps)
        self.pre_feedforward_layernorm = Minigpt4oRMSNorm(self.hidden_size, eps=config.rms_norm_eps)
        self.post_feedforward_layernorm = Minigpt4oRMSNorm(self.hidden_size, eps=config.rms_norm_eps)

        self.hidden_size_per_layer_input = config.hidden_size_per_layer_input
        self.act_fn = ACT2FN[config.hidden_activation]

        self.altup = Minigpt4oTextAltUp(config)
        self.laurel = Minigpt4oTextLaurelBlock(config)
...
class Minigpt4oPreTrainedModel(PreTrainedModel):
    ...
    _can_record_outputs = {
        "hidden_states": Minigpt4oTextDecoderLayer,
        "attentions": Minigpt4oTextAttention,
    }

    def _init_weights(self, module):
        super()._init_weights(module)
        if isinstance(module, Minigpt4oAudioCumulativeGroupNorm):
            module.weight.data.fill_(1.0)
        elif isinstance(module, Minigpt4oAudioAttention):
            module.per_dim_scale.data.zero_()
        elif isinstance(module, Minigpt4oTextAltUp):
...
class Minigpt4oTextModel(Minigpt4oPreTrainedModel):
    ...
    def __init__(self, config: Minigpt4oTextConfig):
        ...
        # Minigpt4o downcasts the below to bfloat16, causing sqrt(3072)=55.4256 to become 55.5. See https://github.com/huggingface/transformers/pull/29402
        self.embed_tokens = Minigpt4oTextScaledWordEmbedding(
        ...
        self.rotary_emb = Minigpt4oTextRotaryEmbedding(config=config)
        ...
        self.rotary_emb_local = Minigpt4oTextRotaryEmbedding(config=config)
...
class Minigpt4oForCausalLM(Minigpt4oPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs,
        ...
        ```python
        >>> from transformers import AutoTokenizer, Minigpt4oForCausalLM

        >>> model = Minigpt4oForCausalLM.from_pretrained("google/minigpt4o-2-9b")
        >>> tokenizer = AutoTokenizer.from_pretrained("google/minigpt4o-2-9b")
        ...
        if self.training and self.config._attn_implementation != "eager":
            logger.warning_once(
                "It is strongly recommended to train Minigpt4o models with the `eager` attention implementation "
...
class Minigpt4oModel(Minigpt4oPreTrainedModel):
    ...
    def forward(
        ...
        **lm_kwargs,
        ...
        >>> model = Minigpt4oForConditionalGeneration.from_pretrained("google/minigpt4o3n2-3b-mix-224")
        >>> processor = AutoProcessor.from_pretrained("google/minigpt4o3n2-3b-mix-224")
...
class Minigpt4oForConditionalGeneration(Minigpt4oPreTrainedModel, GenerationMixin):
    _checkpoint_conversion_mapping = {}
    _tied_weights_keys = ["lm_head.weight"]
    base_model_prefix = "model"
    ...
    def forward(
        ...
        **lm_kwargs,
        ...
        >>> model = Minigpt4oForConditionalGeneration.from_pretrained("google/minigpt4o-3-4b-it")
        >>> processor = AutoProcessor.from_pretrained("google/minigpt4o-3-4b-it")
